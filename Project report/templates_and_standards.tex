...
The group decided to make templates and standards for the most relevant document types. Even though it took some time to create these in the beginning, we believed it would benefit us over time.

\subsubsection{Templates}
We made templates for agendas and minutes of meetings, as well as for the weekly documents. These are located in Appendix \ref{ch:temp}. The template made the work for our agenda and minutes responsible a lot easier.
...
\newline
We used Google Docs to organize our files so that our entire team could look at and change all the documents we had whenever they wanted. We had one folder that contained our entire project and in that folder we had subfolders to make the best overview possible. We tried not to have too many files in each folder so that finding a specific document became as easy as possible.

\subparagraph{Git}\hfill 
\newline
We also used git so that it was easy to share all the code and documents as well. When using \gls{git} all the team members had access to change, read and use all the code files whenever they felt like it.
...
\newline
All the files in our project had a name that started with the date they were created so that they’ll be easier to find. The name also contained one or two words that explained what the documents consisted of.
