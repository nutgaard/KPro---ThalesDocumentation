...
\newpage
	...
	\subsection{Modifiability}
		Even though modifiability was not one of the choosen quality attributes, it has had a significant impact on the overall architecture and is therefore included here. In 'Software Architecture in Practice'\cite[p. 111]{bib:archi} there are three main areas for modifiability tactics listed, which each has several tactics. The ones used by XOXOmail are listed in table~\ref{tab:modtac} on page~\pageref{tab:modtac}.
		\begin{table}[H]
			\begin{description}
