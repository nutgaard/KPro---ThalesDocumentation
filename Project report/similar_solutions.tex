...
\newline
\newline
If, for example,  Bob want to send a message to Alice, he first have to get Alice's public key. Her public key is available to everyone who wants it, and this is the reason why it is called a public key. Bob can use Alice's public key to encrypt the message he want to send her. Does this give any security? Yes it does, as there is a mathematical relation between the private and public key, only Alice can decrypt and read the message. It is nearly impossible or very expensive to calculate the private key, and we can therefore guarantee that only Alice will see the message.
